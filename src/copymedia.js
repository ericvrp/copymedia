...
const callbackFunction = (name, data) => {
  if (!callbackFunctions[name])
    return console.log(`error: callbackFunction "${name}" not found`);
...
const readHistoryCache = () => {
  ...
  const historyPathnames = fs
    .readdirSync(historyFolder)
...
const getCopyItems = (media) => {
  ...
  const sourcePathnames = getAllFiles(media.source).filter((f) =>
    media.extensions.includes(path.extname(f))
  );
  sourcePathnames.forEach((sourcePathname) => {
    ...
    if (!historyCache[cacheString]) {
      ...
      const destinationBaseDirname = path.join(
        media.destination.path,
        `${date.getFullYear()}`,
        yyyymmdd(date) + (projectName ? " " + projectName : "")
      );
      const destinationDirname = path.join(
        destinationBaseDirname,
        media.destination.postfix
      );
...
const createThumbnail = async (copyItem) => {
  ...
  const thumbnailPathname =
    path.join(__dirname, "thumbnails", nCreatedThumbnails.toString()) + ".jpg";
  const createThumbnailCmd = path
    .join(
      __dirname,
      copyItem.isMovie
        ? config.thumbnail.taskForMovie
        : config.thumbnail.taskForPhoto
    )
...
const copyAllMedia = async (_projectName, _callbackFunctions, _config) => {
  ...
  log(
    `Copy ${copyItems.length} files with a total of ${GBtotal.toFixed(2)} GB`
  );
  ...
  for (const copyItem of copyItems) {
    ...
    log(
      `${mmss(t)} ${percentage(done * 100)} ETA in ${mmss(eta)} (${
        copyItem.baseName
      }) [${((MBcopied * 1000) / t).toFixed(2)} MB/s]`
    );
    ...
    if (!config.simulate) {
      ...
      if (!(await existsAsync(copyItem.destinationBaseDirname))) {
        log(`Create folder ${copyItem.destinationBaseDirname}`);
        await mkdirAsync(copyItem.destinationBaseDirname, { recursive: true });
      }
      ...
      writeFileAsync(copyItem.destinationPathname, content).then(() =>
        createThumbnail(copyItem)
      );
    } // else simulate

    copiedSize += copyItem.size;
    historyCache[copyItem.cacheString] = newHistoryCache[
      copyItem.cacheString
    ] = true;
  } //); // next copyItem

  if (!config.simulate && copyItems.length > 0) {
    newHistoryCache["_destinationBaseDirname"] =
      copyItems[0].destinationBaseDirname;
    // log(newHistoryCache);
  ...
  log(
    `${mmss(new Date() - startTime)} ${percentage(100)} of ${GBtotal.toFixed(
      2
    )} GB copied`
  );
...
global.runAsCli =
  !process.argv[0].includes("electron.exe") &&
  !process.argv[0].includes("copymedia.exe");
...
if (runAsCli) {
  ...
  copyAllMedia(
    process.argv[2],
    {
      ...
      log: (message) => {
        if (config.progressReport) console.log(message);
      },
      finished: () => {
